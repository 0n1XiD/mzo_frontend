<template>
    <div class="trainer-weeks-page">
        <div class="_container">
            ...
            />
            <div class="trainer-weeks-page__exercise page-block">
...
export default {
    ...
    async asyncData({ params, i18n, $axios, store }) {
        ...
        let cur_week = []
        let weeks = await $axios.$get(`/api/v1/user/get-client-advanced-weeks/?client_id=${client.id}`, {
            headers: { 'Authorization': `Token ${token}` }
        })
        store.commit('trainer/setClientAdvancedWeeks', weeks)
        ...
        let exercises = await $axios.$get(`/api/v1/user/get-client-advanced-day/?client_id=${client.id}&week_id=${cur_week.id}`, {
            headers: { 'Authorization': `Token ${token}` }
        })
        let old_exercises = exercises
        let old_name = cur_week.name
        let old_description = cur_week.description
        return {client, trainer, token, cur_lang, exercises, cur_week, old_exercises, old_name, old_description, weekIsOpenForClient}
    },
    methods: {
