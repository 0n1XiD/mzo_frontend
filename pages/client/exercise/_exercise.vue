  ...
  <div class="client-exercise">
    <div class="_container">
      ...
      <div class="main__title">
        <div class="main__title main__title_name " v-if="new Date(client.complete_day_time).getDate() < new Date().getDate() || client.complete_day_time == null || cur_week.complete_count == 0 || cur_week.is_additional">
          {{ cur_week.name }} <span>({{$t('exercise.day')}} {{ cur_week.complete_count + 1}})</span>
        </div>
        <div class="main__title main__title_name " v-else>
          {{ cur_week.name }} <span>({{$t('exercise.day')}} {{ cur_week.complete_count}})</span>
        </div>
    ...
    <StatisticalGroup 
      v-if="showStatistical && (client.status == 'client' || client.status == 'mzo_client')" 
      :completeTimes="completeTimes" 
      @close="showStatistical = false"
      @complete="completeQuetions()"
    />

    <MzoStatisticalGroup 
      v-if="showStatistical && client.status == 'online_client'" 
      :completeTimes="completeTimes" 
      @close="showStatistical = false"
      @complete="completeQuetions()"
    />
    ...
    <PopupCompleteDay
      v-if="showCompleteDay"
      :client="client"
      :day="completeTimes"
      :week="cur_week_index"
      @close="showCompleteDay = false"
    />
...
export default {
  ...
  async asyncData({ $axios, i18n, params, store, redirect, require }) {
    ...
    cur_week = weeks.find(x=>x.id===parseInt(params.exercise))
    cur_week_index = store.state.client.client_weeks.weeks.findIndex(x=>x.id===parseInt(params.exercise))


    let exercises = []
    let active_exercises = null
    let time = null
    if (cur_week.is_additional){
      exercises = cur_week.week.exercises
      active_exercises = exercises[cur_week.complete_count_exercises]
      active_exercises.ex_number = cur_week.complete_count_exercises + 1
      time = active_exercises.exercise.duration
    } else {
      if(client.type != 'pregnant'){
      ...
      for(let i in exercises){
        if (new Date(client.complete_day_time).toISOString().split('T')[0] >= new Date().toISOString().split('T')[0]) {
          if(exercises[i].repeat_count == 0){
    ...
    console.log(active_exercises)
    return {client, token, exercises, cur_lang, cur_week, active_exercises, time, cur_week_index}
  },
  methods: {
    startTimer () {
      this.isRunning = true
    ...
    async previousExercise(){
      ...
      if (new Date(this.client.complete_day_time).toISOString().split('T')[0] >= new Date().toISOString().split('T')[0]) {
        params.is_repeat = true
      }
      if(this.client.type != 'pregnant'){
        await this.$axios.$post('/api/v1/user/previous-client-day/', params, { 
          headers: { Authorization: `Token ${this.token}` } 
        }).catch(e => {
          console.log(e.response)
        })
      } else{
        await this.$axios.$post('/api/v1/user/previous-client-period-day/', params, { 
          headers: { Authorization: `Token ${this.token}` } 
        }).catch(e => {
          console.log(e.response)
        })  
    ...
    async nextExercise(complete=false){
      ...
      let params = {
        ...
        is_additional: this.cur_week.is_additional,
        is_repeat: false
      }
      if(this.cur_week.is_additional){
      ...
      if(this.client.type != 'pregnant'){
        ...
        await this.$axios.$post('/api/v1/user/complete-client-period-day/', params, { 
          headers: { Authorization: `Token ${this.token}` } 
        }).catch(e => console.log(e.response)) 
      }
    ...
    async ShowQuestionnaire(){
      if (!this.cur_week.is_additional && this.client.type != 'pregnant' && (this.completeTimes == 7 || this.completeTimes == 4)){
        let statistic = []
        if(this.$store.state.client.statistic){
          statistic = this.$store.state.client.statistic
        } else {
          statistic = await this.$axios.$get(`/api/v1/user/get-client-statistics/?client_id=${this.client.id}`, {
            headers: { 'Authorization': `Token ${this.token}` }
          }) 
          this.$store.commit('client/setStatistic', statistic)
        }
        this.showStatistical = true
      } else {
        this.showCompleteDay = true
      }
    },
    async getPopupBillet(){
      if (this.completeTimes <= 8 && this.client.type != 'pregnant'){
        let popup_billet_content = []
        if (this.$store.state.userInfo.status == 'client' || this.$store.state.userInfo.status == 'mzo_client'){
          popup_billet_content = await this.$axios.$get(`/api/v1/site_content/get-popup/?week_num=${this.$cookies.get('w_w')}&day_num=${this.$cookies.get('w_d')}&client_type=${this.$store.state.userInfo.status}`, {
            headers: { 'Authorization': `Token ${this.token}` }
          }).catch(e => {
            console.log(e.response)
          })
        }
        if (this.$store.state.userInfo.status == 'online_client'){
          popup_billet_content = await this.$axios.$get(`/api/v1/site_content/get-popup/?week_num=${this.$cookies.get('w_w')}&day_num=${this.$cookies.get('w_d')}&client_type=${this.$store.state.userInfo.status}&rate_id=${this.$store.state.userInfo.rate.id}`, {
            headers: { 'Authorization': `Token ${this.token}` }
          }).catch(e => {
            console.log(e.response)
          })
        }
        this.$store.commit('client/setHtmlPopupContent', popup_billet_content)

        let week_day_id = this.completeTimes + 1
        if (week_day_id == 8){
          week_day_id = 1
        }
        this.$cookies.set("w_d", week_day_id);
      } else if(this.client.type == 'pregnant' && this.completeTimes <= 32){
        let week_day_id = this.completeTimes + 1
        if (week_day_id == 8){
          week_day_id = 1
        }
    ...
    completeDay(){
      if(this.cur_week.is_additional){
          ...
          for(let i in this.$store.state.client.client_weeks.additional_weeks){
            if (this.$store.state.client.client_weeks.additional_weeks[i] == this.cur_week){
              this.$store.state.client.client_weeks.additional_weeks[i].complete_count += 1
              break
            }
          }
        } else {
          if(new Date(this.client.complete_day_time).toISOString().split('T')[0] < new Date().toISOString().split('T')[0] || this.client.complete_day_time == null){
            this.completeTimes = this.cur_week.complete_count + 1
            for(let i in this.$store.state.client.client_weeks.weeks){
              if (this.$store.state.client.client_weeks.weeks[i] == this.cur_week){
                this.$store.state.client.client_weeks.weeks[i].complete_count += 1
                break
              }
            }
          } else{
        ...
        this.$store.commit('client/setClientWeeks', this.$store.state.client.client_weeks)
        if(new Date(this.client.complete_day_time).toISOString().split('T')[0] < new Date().toISOString().split('T')[0] || this.client.complete_day_time == null){
          if(this.completeTimes >= 7 && this.client.type != 'pregnant' && !this.cur_week.is_additional) this.completeWeek()
          if (this.client.type == 'pregnant' && this.completeTimes >= 31 && !this.cur_week.is_additional) this.completeWeek()
    ...
    async completeWeek(){
      try{
        let next_week = this.$store.state.client.client_weeks.weeks
        next_week[this.cur_week_index + 1].is_available_for_trainer = true
        if (this.client.status == 'online_client' && this.client.rate.auto_week_complete){
          next_week[this.cur_week_index + 1].is_available_for_client = true
        }
        this.$cookies.set("w_w", this.cur_week_index + 2);
        let params = {
        ...
        this.getPopupBillet()
        this.$store.commit('client/setDayIndex', this.$cookies.get('w_d'))
      } catch(e) {
        console.log(e.response)
      }
    },
