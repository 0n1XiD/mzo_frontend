<template>
    <div class="wrapper">
        <div class="popup-statistical-group" :class="{'popup-statistical-group_small': statisticalPage == 1}">
            ...
            />
            <MzoStatisticalPhotos
                v-if="statisticalPage == 4"
                :staticInfoData="staticInfoData"
                @click="changePage($event, 5)"
            />
            <MzoStatisticalAllDone v-if="statisticalPage == 5" @complete="completeQutions()"/>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async changePage(e, page){
            ...
            if(this.statisticalPage == 4){
                ...
                let have_photos = false
                for (let i in this.staticInfoData.photos){
                    if(this.staticInfoData.photos[i].src){
                        have_photos = true
                        break
                    }
                }
                if (have_photos){
                    let photo_params = {
                        photos: this.staticInfoData.photos,
                    }
                    await this.$axios.$post('/api/v1/user/add-client-photo/', photo_params, { 
                        headers: { Authorization: `Token ${this.$store.state.userInfo.token}` } 
                    })
                }
            }
    ...
    components: {
        MzoStatisticalCongratulations: () => import(`@/components/mzo-client-statistical-collection/MzoStatisticalCongratulations.vue`),
        MzoStatisticalPainLevel: () => import(`@/components/mzo-client-statistical-collection/MzoStatisticalPainLevel.vue`),
        MzoStatisticalStatus: () => import(`@/components/mzo-client-statistical-collection/MzoStatisticalStatus.vue`),
        MzoStatisticalPhotos: () => import(`@/components/mzo-client-statistical-collection/MzoStatisticalPhotos.vue`),
        MzoStatisticalAllDone: () => import(`@/components/mzo-client-statistical-collection/MzoStatisticalAllDone.vue`)
    }
}
</script>
