<template>
    <div class="posture-photos new-client">
        <div class="new-client__title">{{$t('posturePhotos.photosTitle')}}</div>
        <div class="new-client__subtitle">
            {{$t('posturePhotos.photosSubtitle')}}
        </div>
        <div class="new-client__content posture-photos__content">
            ...
            />
            <div class="photos">
                <div class="photos__item" v-for="(photo, index) in localStaticInfoData.photos" :key="`photo-${index}`">
...
export default {
    ...
    methods: {
        uploadImage(e, index){
            let saveSrc = ''
            if(this.localStaticInfoData.photos[index].src != null && this.localStaticInfoData.photos[index].src != ''){
                saveSrc = this.localStaticInfoData.photos[index].src
            }
            try{
                ...
                if(size <= 10000000){
                    ...
                    reader.onload = e =>{
                        this.localStaticInfoData.photos[index].src = e.target.result;
                        this.cropPhoto = {index: index, photo: e.target.result}
                        this.isCrop = true
                    };
                } else {
                    this.errorPhoto = true
                    this.localStaticInfoData.photos[index].src = saveSrc
                }
            } catch {}
        },
        async saveUploadPhoto(e){
            this.localStaticInfoData.photos[e.index].src = e.photo
            this.localStaticInfoData.photos[e.index].top_line_first = e.first_line
            this.localStaticInfoData.photos[e.index].top_line_second = e.second_line
        },
        callback: function(e) {
